...
def gen_image(model, image, bsz, seed, num_iter=12, choice_temperature=4.5,per_iter=True):
    ...
    for step in range(num_iter):
        ...
        # Loop through each pair of images (even, odd) and collect the necessary slots
        for i in range(0, 32, 2):  # Step by 2 to handle pairs
            even_slots = slots[i, :3,:]  # First 3 slots of even-indexed image
            odd_slots = slots[i + 1, :4,:]  # First 4 slots of odd-indexed image
            # Concatenate along the slot dimension (assuming it's the second dimension)
            combined_slots = torch.cat((even_slots, odd_slots), dim=0)
            new_slots_list.append(combined_slots)

        # Concatenate all new slots to form the new slots tensor
...
for batch, data in iterator:
    ...
    with torch.no_grad():
        ...
        # save img
        for b_id in range(int(args.batch_size)):
          if(b_id<int(args.batch_size/2)):
