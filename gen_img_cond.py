...
#########################
    ...
    token_all_mask = torch.ones(bsz, unknown_number_in_the_beginning+1, device=device).float().cuda()  # No tokens are masked
    token_drop_mask = torch.zeros(bsz, unknown_number_in_the_beginning+1, device=device).float().cuda()  # No tokens are dropped

    x = model.mask_token.expand(bsz, 1 + 256, 768).clone().cuda()
    slots = model.mask_token.expand(bsz, model.slot_attention.num_slots, 768).clone().cuda()
...
# Define the number of initial slots to be initialized for each sample
    ...
    for i in range(bsz):
        ...
        while initialized_slots < num_initial_slots:
            ...
            if random_slot_index not in replaced_slots[i]:
                ...
                slots[i, random_slot_index] = cluster_centers(torch.tensor(random_centroid_index).cuda())
    ...
    for iteration in range(model.slot_attention.num_slots):
        ...
        for i in range(bsz):
            ...
            top_3_options = [(idx, probs[i, idx].max().item()) for idx in top_3_indices]
            print(f"Iteration {iteration}, Batch Item {i}: Top 3 Options (Slot Index, Probability): {top_3_options}")
            ...
            for slot_index in sorted_indices:
                if slot_index not in replaced_slots[i]:
                    ...
                    print(f"Cluster Center Index Selected: {most_probable_center_idx}")
