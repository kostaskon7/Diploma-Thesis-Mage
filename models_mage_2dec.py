...
class MaskedGenerativeEncoderViT(nn.Module):
    ...
    def forward(self, imgs,mask_crf):
        ...
        # slots_pool = slots_pool + decoder_pos_embed_learned_pool
        ...
        with torch.cuda.amp.autocast(enabled=False):
            if self.training:
                ...
                mask_crf_onehot = torch.nn.functional.one_hot(mask_crf, num_classes=self.slot_attention.num_slots).permute(0,3,1,2).to(dtype=torch.float32)
