    ...
    for step in range(num_iter):
        ...
        if step == 0:
          ...
          logits = logits[:, model.slot_attention.num_slots+1:, :codebook_size]
                    ...
                    # token embedding
          input_embeddings = model2.token_emb(token_indices)
          ...
          # decoder
          logits = model2.forward_decoder(x, token_drop_mask, token_all_mask)
        ...
        if(with_mask_vis):
            ...
            z_q = model2.vqgan.quantize.get_codebook_entry(reconstructed_indices, shape=(batch_size, 16, 16, codebook_emb_dim))
            gen_images_batch = model2.vqgan.decode(z_q)
...
parser.add_argument('--ckpt', type=str,
                    help='checkpoint')
parser.add_argument('--ckpt_new', type=str,
                    help='checkpoint')
...
model2 = models_mage_original.__dict__[args.model](norm_pix_loss=False,
                                         mask_ratio_mu=0.55, mask_ratio_std=0.25,
                                         mask_ratio_min=0.0, mask_ratio_max=1.0,
                                         vqgan_ckpt_path=vqgan_ckpt_path)
model.to(0)
...
checkpoint2 = torch.load(args.ckpt_new, map_location='cpu')
